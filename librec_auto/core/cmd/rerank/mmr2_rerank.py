...
class Rerank_Helper():
    ...
    def get_protected_set(self, item_features):
        return set((item_features[(item_features['feature'] == self.protected)
                                  & (item_features['value'] == 1)].index).tolist())

    def set_rerank_helper(self, args, config):
        # basic info
        ...
        try:
            self.protected_set = self.get_protected_set(item_helper.item_feature_df, item_helper)
        except:
            pass
...
class Item_Helper():
    ...
    def set_item_helper(self, item_feature_df):
...
class Reranker():
    # def fun(self):
    #     def no_op(rec, item_helper, rerank_helper, user_helper):
    #         return rec, item_helper, rerank_helper, user_helper
    #     return no_op


    def __init__(self, rating, training, rerank_helper, item_helper, scoring_function):
        self.rating = rating
    ...
    def reranker(self):
        ...
        return df, self.rerank_helper, self.item_helper
    ...
    def greedy_enhance(self):
        ...
        for k in range(num_item - 1):
            ...
            # scoring function
            scores, self.item_helper, self.rerank_helper, self.user_helper = self.scoring_function(rec, self.item_helper, self.rerank_helper,
                                                                               self.user_helper)
...
class MMR(Reranker):
    ...
    def __init__(self, rating, training, rerank_helper, item_helper):
        Reranker.__init__(self, rating, training, rerank_helper, item_helper, self.fun())
...
def execute(rerank_helper, item_helper, pat, file_path, split_path, dest_results_path):
    tr_df = None
    ...
    re_ranker = MMR(rating_df, tr_df, rerank_helper, item_helper)

    reranked_df, rerank_helper, item_helper = re_ranker.reranker()
...
def main():
    ...
    rerank_helper.set_rerank_helper(args, config)
    ...
    for file_path in result_files:
        ...
        rerank_helper, item_helper, pat, file_path, split_path, dest_results_path))
        p.append(p1)
        p1.start()

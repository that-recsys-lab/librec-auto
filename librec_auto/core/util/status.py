...
class Status():
    ...
    def __init__(self, sub_paths):
        ...
        if status_path.exists():
            ...
            self._message = extract_from_path(
                self._status_xml, ['librec-auto-status', 'message'])
            if self._subpaths.get_path('log').exists():
                self._log = LogFile(self._subpaths)
            else:
                self._log = None
            params = extract_from_path(self._status_xml,
                                       ['librec-auto-status', 'param'])
            if params != None:
    ...
    def save_status(msg, exp_count, config, paths):
        ...
        status_front = Status._TEMPLATE_FRONT.format(msg, exp_count,
        ...
        for var_elem in var_elems:
            ...
            status_params = status_params + Status._TEMPLATE_LINE.format(
                var_name, var_value)
